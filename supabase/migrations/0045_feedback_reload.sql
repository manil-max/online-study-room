-- 0045: Feedback şema ensure + PostgREST schema cache reload (WP-184)
--
-- 0044 feedback tablolarını/policy/bucket'ı idempotent kuruyordu ama
-- `NOTIFY pgrst, 'reload schema'` yoktu. Sonuç: SQL Editor'da tablo varken
-- REST API hâlâ "schema cache" / schema_missing hatası verebiliyordu.
--
-- Bu dosya 0044 gövdesini güvenle tekrarlar (iki kez çalıştırılabilir) ve
-- sonunda PostgREST'e şema önbelleğini yenilemesini söyler.
--
-- Geri alma (Rollback): NOTIFY geri alınamaz (zararsız). Tablo/policy drop
-- üretimde tehlikelidir — yalnız bilinçli admin operasyonu.
...
-- is_super_admin (0018/0044 ile aynı niyet; zaten varsa replace)
...
-- WP-184 kök neden: PostgREST REST API şema önbelleğini yenile.
-- Tablo SQL'de var olsa bile cache eskiyse istemci schema_missing görür.
