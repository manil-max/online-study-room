...
| **Tem 12** | **Merge otomasyonu = A.** WP başına dal → push → PR → CI (`analyze`+`test`) → yeşilse auto-merge (squash). Merge elle yapılmaz; testten geçmeyen kod main'e giremez. Kurulum WP-39; auto-merge/branch-protection ayarını kullanıcı açar. WP dalları public repo'ya push edilir. |
