...
- Remote'a uygulanmış migration immutable'dır; düzeltme yeni ileri migration ile gelir.
  - 🔴 **Tek istisna — "yarı uygulanmış zincir" (2026-08-09, WP-549/`0124` ile kanıtlandı):** bir migration *bazı* ortamlara uygulanmış ama başka bir ortamda **kırılıyorsa**, ileri migration çözüm **değildir** — `db push` kırık migration'ı önce koşturur ve zincir hiç ilerlemez. Bu durumda kırık migration **yerinde** düzeltilir. Şartlar: (1) sapma migration'ın kendi başlığında, commit mesajında ve `deploy-contract.json`'da yazılır; (2) uygulanmış ortamda düzeltmenin veri etkisi olmadığı **kanıtlanır** (`0124`'te kanıt: staging'de eşleşen satır olmadığı için orijinal ifade sıfır satıra dokunmuştu — zaten bu yüzden geçmişti); (3) düzeltmeyi ölçen bir test aynı commit'te gelir. Bu üçü yoksa istisna yoktur.
- **Taze kurulumda sıfır satıra dokunan hiçbir ifade "sınandı" sayılmaz.** `where ... is null` biçimindeki backfill'ler boş veritabanında hep geçer; kapı yeşil yanar, kusur production verisinde patlar (`0124`, 27. ifade). Böyle bir ifade yazan migration, satırı **elde kuran** bir pgTAP testi getirmek zorundadır.
- Ürün davranışını bağlayan yazılı kararlar: `docs/URUN-POLITIKALARI.md` (regresyonda eski düzen seçenek kalır · kalıcı ücretsiz+reklamsız · zorlama yok · durdur≠sil · açılışta yalnız TR · araç yığınına girilmez).
