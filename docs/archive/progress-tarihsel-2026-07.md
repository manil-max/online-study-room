# progress-tarihsel-2026-07.md — Arşiv (tarihsel WP kartları)

> `progress.md`'den **2026-07-19'da ayrıldı** (token optimizasyonu — ajanlar her oturumda 1700 satır okumasın).
> Buradakiler **TAMAMLANMIŞ / tarihsel** WP kartlarıdır — **yeniden claim edilmez**, güncel durum kaynağı değildir.
> Canlı durum: `../progress.md` (WP Durum Dizini + Aktif Çalışma Kaydı).
> İçerik: Tarihsel uygulama kartları (WP-103–109, WP-93), Play Store detay kartları (WP-110–124), park (Test için bekleyenler) detay kartları, Tamamlanan detay kartları, Son Teslim Notları, **Yeni Özellik Turu Aşama A'da kod/test tamamlanan kartlar (WP-286–291, 293)**.
...
- **Düzeltme:** `_awaitAuthRetryWindow` + `_cancelAuthRetryWindow` (Timer+Completer); `ref.onDispose` iptal.
- **Kapsam dışı:** FGS/widget/MainActivity/notification SSOT davranışı değişmedi; WP-134–137 cihaz QA kapanmadı.
- **Kanıt:** `flutter analyze` 0 · `timer_mode_controls_test` 3/3 · full `flutter test` **525 passed, 0 fail**
- **Commit:** (bu) · Push yok
...
- **Yapılmadı:** Edge deploy, Console form, AAB upload, versionCode bump, cihaz testi.
- **Kanıt:** `Kodda doğrulandı` (docs) · cihaz/Console **sahip**
- **Commit:** (bu)
...
## Yeni Özellik Turu — Aşama A: kod/test tamamlanan kartlar (2026-07-24/25)

> Bu kartlar `progress.md`'den **2026-07-25'te** taşındı. Kod/test tamam; kalan iş
> yalnız **cihaz QA** ve canlı dosyanın `Cihaz QA Kuyruğu` tablosunda izleniyor.
> Yeniden claim edilmez — kart içerikleri o günkü gerçeği taşır, güncellenmez.
>
> Kapsam: WP-293 (Gate 0), WP-286, WP-287, WP-288, WP-289, WP-290, WP-291.
