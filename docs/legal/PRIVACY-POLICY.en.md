...
|---|---|---|
| Account | email, auth | Sign-in, security |
| Profile | display name, avatar, camp animal | Social / class |
| Study | session times, subjects, goals | Stats, XP, groups |
...
- **Supabase** (prefer EU region): Auth, DB, storage, edge functions  
- **Sentry** (only when build-enabled and user telemetry on): crash reports  
- **GitHub Releases** (non-Play sideload channels): update packages  
...
Access, correction, deletion requests, disable telemetry (Settings → Privacy & legal).

## 6. Changes

Version/date updated in-app legal center.
