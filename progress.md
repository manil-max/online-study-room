...
- **Son WP numarası:** **253** (WP-249 = v42 stable release hazırlığı `94947fc`; WP-250 = durdurma çift-sayımı settling modeli; WP-251 = native kuyruk idempotency; WP-252 = native pomodoro ExactAlarm **onay bekliyor**; WP-253 = UX sıralama ikonu + manuel çakışma **karar bekliyor**). **Sıradaki boş numara WP-254.** (WP-221–224 geçmişte kullanılmış; kurtarma WP-225–232; WP-233/234 beta-v4202; WP-235–239 saha turu 2; WP-241/242 beta-v4204; WP-243/244 beta-v4205; WP-245–248 beta-v4206; WP-249 v42 stable; WP-250–253 sayaç düzeltme programı → `docs/WP-250-253-SAYAC-DUZELTME-PLANI.md`.)
...
### Claude Lane
- **Durum:** [~] Aktif
- **Faz/WP:** WP-250 (durdurma çift-sayımı — settling modeli) + WP-251 (native kuyruk idempotency). Öncesi: WP-245/246/247 (durdurma kök neden 3'lü fix) + WP-248 (beta-v4206 yayını).
- **Aşama:** WP-250 kod tamam (analyze 0 issue, **655 test yeşil**; yeni regresyon testleri fix'siz DÜŞÜYOR — kırmızı-yeşil ispatı yapıldı). WP-251 geliştiriliyor. beta-v4206 yayın kaydı hazır — **cihaz doğrulaması bekliyor.**
- **SAHİP yollar:** `app/lib/core/stats/study_stats.dart`, `app/lib/data/providers/study_providers.dart`, `app/lib/features/classroom/widgets/study_timer_card.dart`, `app/lib/features/classroom/widgets/focus_timer_screen.dart`, `app/android/**/timer/TimerStateStore.kt`, `app/test/core/study_stats_test.dart`, `app/test/features/timer_background_reconcile_test.dart`, `app/test/features/classroom/study_timer_card_stop_test.dart`, `CHANGELOG.md`, `app/assets/release_notes.json`, `progress.md`
...
- **Cihaz QA listesi (stable öncesi):** ① bildirimden Başlat→uygulama içi Durdur (WP-233); ② başarım ilerleme/taç kademe (WP-234); ③ 1s kayıtlı+1s canlı→Durdur = 2s (3s değil), kapat-aç gerekmeden (WP-239/250); ④ grafiklerde X her gün + Y ekseni (WP-237); ⑤ **arka planda bildirimden Durdur → 5 dk sonra uygulamayı aç**: toplam yalnız gerçek çalışma kadar artmalı (WP-250 ölü zaman); ⑥ **uçak modunda 2 oturum kuyruğa at → ağı aç → uygulamayı 2 kez aç**: tam 2 oturum, kopya yok (WP-251); ⑦ pomodoro çalışma→mola geçişinde toplam düşmemeli (WP-250).
- **Dal:** `main`
...
| WP-250 | [x] Cihaz QA bekliyor | 🔴 P0: Durdurma çift-sayımı — DB yazımı (RTT) sırasında canlı süre ikinci kez sayılıyordu (1 sa → 2 sa) ve dondurma hatayı gün boyu kilitliyordu | ✅ settling modeli; freeze mekanizması tamamen kaldırıldı; 3 yeni regresyon testi (fix'siz düşüyor) |
| WP-251 | [~] Geliştiriliyor | 🔴 P1: Native aralık kuyruğu — kısmi hatada başarılı kayıtlar tekrar yazılıyordu (DB'de çift oturum) + toptan silme reconcile sırasında eklenen aralığı kaybediyordu | ✅ native UUID + id-bazlı kısmi silme; repo zaten idempotent (`upsert onConflict:id`) |
...
| WP-253 | [ ] Karar bekliyor | UX: sıralamadaki ateş ikonu (kartta hedef serisi, sıralamada aktif gün serisi — aynı ikon iki anlam) + manuel eklemede bugün/çakışma koruması | ikon kararı ürün sahibinde |
